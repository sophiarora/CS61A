...
class Player(object):
    ...
    def look(self):
        """Taking a look and understanding the environment.

        >>> sather_gate = Place('Sather Gate', 'You are at Sather Gate', [], [])
        >>> sophie = Player('Sophie', sather_gate)
        >>> sophie.look()
        You are currently at Sather Gate. You take a look around and see:
        Characters:
    ...
    def talk_to(self, person):
        ...
        if type(person) != str:
            ...
            person_character = self.place.characters[person]
            print(person, 'says:', person_character.message )
        else:
    ...
    def take(self, thing):
        ...
        if type(thing) != str:
            ...
            thing_in_place = self.place.things[thing]
            self.backpack += [thing_in_place]
    ...
    def unlock(self, place):
        ...
        for item in self.backpack:
            if type(item) == Key:
                key = item
                unlock_place = self.place.get_neighbor(place)
                return key.use(unlock_place)
        if key == None:
