    ...
    try:
        try:
            if len(args) == 1:
                ...
                result = procedure.fn(args[0])
                return result
            elif procedure == create_global_frame().bindings['cons']:
                result = procedure.fn(args[0], args[1])
                return result
            else:
                try:
                    result = nil
                    for e in reversed(args[0:num]):
                        result = procedure.fn(e, result)
                    return result
                except SchemeError as e:
                    result = procedure.fn(args[num-1])
                    for e in reversed(args[0:num-1]):
                        result = procedure.fn(e, result)
                    return result
...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        ...
        while expressions.second != nil:
            exp_list += [expressions.first]
            expressions = expressions.second
        exp_list += [expressions.first]
        num = len(exp_list) - 1
        for exp in exp_list[0:num]:
            scheme_eval(exp, env)
        exp_list.reverse()
        return scheme_eval(exp_list[0], env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 5A
        valuation = expressions.second
        if type(valuation.first) == int:
            env.define(target, valuation.first)
        else:
            target_value = scheme_eval(valuation.first, env)
